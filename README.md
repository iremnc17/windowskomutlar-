...
ipconfig
``` 
- Detaylı ağ bilgilerini listeler
...
netstat
``` 
- Bilgisayarı kapatır veya yeniden başlatır aşağıdaki örnekte yazdığım "shutdown /s" bilgisayarı kapatır
...
tree
``` 
- Dizin yapısını hiyerarşik olarak görüntülemesinin yanında dosyaları da gösterir
...
net user
```
- DNS sunucusuu sorgulaması yapar.
...
start
```
- Dosya türü ilişkilerini gösterir veya değiştirir.
...
set
```
- Bilgisayarın adını görüntüler
![açıklayıcı](/resim%20irem/hostname.png)
```bash
hostname
```
- Güç ayarlarını yönetir.
...
popd
```
- Komut istemi metin ve arka planı renklendirmek için kullanılır [color 0A (yeşil metin siyah arkaplan)]
![açıklayıcı](/resim%20irem/color.png)
```bash
color
``` 
- Komut istemi penceresine bir başlık verir.
...
exit
```
- Yorum satırı ekler (komut dosyalarında açıklama yapmaya yarar).
